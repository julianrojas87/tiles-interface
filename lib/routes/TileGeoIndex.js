...
async function calculateTilesSet(counts, storage) {
    ...
    if (counts[pZ][pX][pY] <= storage.geoIndex.threshold) {
        ...
        console.time(`tile_drilling_${storage.name}`);
        await drillTiles(pZ + 1, pX * 2, pY * 2, counts, storage, tileSet);
        console.timeEnd(`tile_drilling_${storage.name}`);
    }

    // Register average count of tile set
    let actualTileCount = 0;
    tileSet.features.forEach(f => {
    ...
    tileSet["Properties"] = {
        average: counts[pZ][pX][pY] / actualTileCount
    }; 
